(function () {
  ...
  function injectProfileModal() {
    ...
    modal.innerHTML = `
      <div class="hb-modal">
        ...
        <p class="hb-profile-copy">Your name is saved on this device. No password needed.</p>
        <div class="hb-profile-row">
          <label for="hbDisplayNameInput">Display name</label>
          <input id="hbDisplayNameInput" class="hb-input" type="text" maxlength="16">
        </div>
    ...
    document.getElementById("hbSaveProfileBtn").addEventListener("click", saveProfileName);
  }
  ...
  function saveProfileName() {
    ...
    if (nextName.length < 3) {
      feedback.textContent = "Use at least 3 letters or numbers.";
      return;
    }
  ...
  async function fetchLeaderboard(gameId) {
    ...
    try {
      ...
      data.leaderboard.forEach((entry) => {
        const key = String(entry.display_name || "").trim().toLowerCase();
