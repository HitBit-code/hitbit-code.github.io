(function () {
  ...
  const GAME_META = {
    stack:          { label: "HitBit Stack",     unit: "pts", scoreKey: "hitbitHigh",        histKey: null },
    orbfall:        { label: "Orb-Fall",          unit: "pts", scoreKey: "orbFallHighscore",  histKey: "orbFallScores" },
    strafe:         { label: "HitBit Strafe",     unit: "pts", scoreKey: "hitbit_strafe_hs",  histKey: null },
    memory:         { label: "HitBit Memory",     unit: "pts", scoreKey: null,                histKey: "memoryScores" },
    timing:         { label: "Timing Challenge",  unit: "pts", scoreKey: "hitbitHigh",        histKey: "hitbitScoreHistory" },
    visualreaction: { label: "Visual Reaction",   unit: "ms",  scoreKey: "hitbit_highscore",  histKey: null }
  };
  ...
  function getProfileStats() {
    ...
    for (const [id, meta] of Object.entries(GAME_META)) {
      ...
      if (meta.scoreKey) {
        ...
        if (val !== null && val !== "" && !isNaN(Number(val))) best = val;
      } else if (hist.length > 0) {
        // Derive best from history array (e.g. memory)
        best = String(Math.max(...hist));
      }

      if (best !== null) {
        bests.push({ id, label: meta.label, unit: meta.unit, best });
      }
    }
    ...
    if (bests.length === 0) {
      ...
      const grid = `<div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(140px, 1fr)); gap:8px;">
        ${bests.map(b => `
          ...
          ">
            <div style="font-size:0.8rem; color:var(--hb-text-dim); margin-bottom:4px;">${b.label}</div>
